...
type VideoUpsertRow = Omit<ShopVideoRow, "featured_products" | "auto_matched_products"> & {
  featured_products?: number[] | null;
  updated_at?: string;
};

/**
 * Upserts video metadata and always overwrites auto_matched_products (including []).
 */
export async function upsertVideoWithAutoMatch(
  ...
  const { error: matchError } = await supabase
    .from(table)
    .update({ auto_matched_products, updated_at })
