...
export async function GET(request: Request) {
  try {
    ...
    console.log("[haendler/invoice] third-country PDF branch", {
      orderId: order.id,
    ...
    order.line_items.forEach((item, i) => {
      ...
      page.drawText(formatPrice(unitPrice), { x: colX.unit, y, size: 9, font, color: WHITE_TEXT });
      page.drawText(formatPrice(itemTotal), { x: colX.total, y, size: 9, font, color: WHITE_TEXT });
    ...
    if (!isReverseCharge && !isThirdCountryExport) {
      ...
      page.drawText(formatPrice(netAmount), { x: colX.total, y, size: 9, font, color: WHITE_TEXT });
      y -= 16;

      page.drawText("USt. 20%:", { x: 380, y, size: 9, font, color: GREY });
      page.drawText(formatPrice(vatAmount), { x: colX.total, y, size: 9, font, color: WHITE_TEXT });
      y -= 16;
    }
    ...
    if (isThirdCountryExport) {
      console.log(
        "[haendler/invoice] drawing steuerfreie Ausfuhr note (2 lines) at y=",
        y - 20
      );
      y -= 20;
